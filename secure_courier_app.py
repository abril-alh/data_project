...
def main():
    ...
    # Config explanation
    st.sidebar.markdown("API keys are securely stored and not displayed in the UI")
    
    # Add API configuration status indicators
    if weather_key:
        st.sidebar.success("✅ OpenWeatherMap API Key configured")
    else:
        st.sidebar.error("❌ OpenWeatherMap API Key missing")
        
    if news_key:
        st.sidebar.success("✅ NewsAPI.org API Key configured")
    else:
        st.sidebar.error("❌ NewsAPI.org API Key missing")
