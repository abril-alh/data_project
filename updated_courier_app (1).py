...
def get_news(country_code, city, api_key):
    ...
    with st.spinner(f"Fetching local news for {city}, {country_code.upper()}..."):
        api_key = api_key.strip()
        # Corregido: URL correcta para NewsAPI.org (no .ai)
...
def find_gas_stations(lat, lon):
    ...
    # Simulate different gas station availability based on time
...
def main():
    ...
    # API Keys - Removed hardcoded keys for security
    weather_key = st.sidebar.text_input("OpenWeatherMap API Key", value="", type="password")
    news_key = st.sidebar.text_input("NewsAPI.org API Key", value="", type="password")
    
    # Show reminder if keys are empty
    if not weather_key or not news_key:
        st.sidebar.warning("⚠️ Please enter your API keys to get weather and news data")
    ...
    # Main content
    st.markdown('<div class="main-header">🗺️ ZONE INFORMATION</div>', unsafe_allow_html=True)
    ...
    # Generate data
    weather_success, weather_data = get_weather(city, weather_key) if weather_key else (False, "API key required")
    news_success, news_data = get_news(country, city, news_key) if news_key else (False, [{"title": "API key required for news data", "url": "#", "source": "System"}])
    
    # Get gas station data instead of delivery load
    ...
    with col2:
        ...
        station_class = "info-box"
        if stations_level == "Low":
            station_class += " danger"
        elif stations_level == "Medium":
            station_class += " warning"
        else:
            station_class += " success"
            
        st.markdown(f"""
        <div class="{station_class}">
...
# Añadido: Asegurar que la aplicación se ejecuta
